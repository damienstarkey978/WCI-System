 ...
 * vendors, and clients. Team is read-only here — staff accounts are
 * provisioned via Clerk + the seed script, not a form in this UI — while
 * Vendors and Clients link through to their own management pages.
 */
        ...
        <ul className="mt-2 divide-y" style={{ borderColor: "var(--bt-border)" }}>
          {team.map((member) => (
            <li key={member.id} className="flex items-center justify-between py-2 text-sm">
              ...
              <span className="text-xs text-[var(--bt-muted)]">{ROLE_LABEL[member.role] ?? member.role}</span>
            </li>
          ))}
        </ul>
      </section>
